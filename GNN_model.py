...
def multi_layer_fc_fn(Ks=[300, 64, 32, 64], num_classes=3, is_logits=False, num_layers=4):
...
def prediction_fc_fn(Ks=[300, 64, 32, 64], num_classes=3, is_logits=False, num_layers=4):
    assert len(Ks) == num_layers
    linears = []
    for i in range(1, len(Ks)):
        linears += [
...
class ClassAwarePredictor(nn.Module):
    def __init__(self, num_classes, box_encoding_len):
        ...
        self.loc_fn=prediction_fc_fn(Ks=[300, 300, 128,32], num_layers=4, num_classes=box_encoding_len, is_logits=True)
        
        
    def forward(self, features):
        logits = self.cls_fn(features)
        
        box_encodings = self.loc_fn(features)#.unsqueeze(1)####DO I NEED UNSQUEEZE
...
class MultiLayerFastLocalGraphModelV2(nn.Module):
    ...
    def loss(self, logits, labels, pred_box, gt_box, valid_box, 
        ...
        all_loc_loss = huger_loss(pred_box, gt_box.squeeze())#.squeeze()
        #all_loc_loss = all_loc_loss * valid_box.squeeze(1)
        #higher weight (10) for weld region
        loss_weight_mask = valid_box.squeeze(1)*9+1
        ...
        loc_loss = mean_loc_loss.sum() / num_endpoint
        #loc_loss = mean_loc_loss
        
        loss_dict = {}
        loss_dict['classwise_loc_loss'] = loc_loss
